...
class CategoryMultiSelectBottomSheet : BottomSheetDialogFragment() {
    ...
    // Hardcoded categories matching the wizard for now
    private val categoriesMap = mapOf(
        "general" to listOf("Uncategorized"),
        "electronics" to listOf("Mobiles", "Laptops", "Accessories", "Home Appliances"),
        "fashion" to listOf("Men's Clothing", "Women's Clothing", "Shoes", "Watches"),
        "home" to listOf("Furniture", "Decor", "Kitchen", "Bedding"),
        "beauty" to listOf("Makeup", "Skincare", "Haircare", "Fragrances"),
        "sports" to listOf("Fitness", "Outdoor", "Team Sports", "Cycling")
    )

    private val allCategories = categoriesMap.keys.toList()
    ...
    inner class CategoryAdapter(private val categories: List<String>) : RecyclerView.Adapter<CategoryAdapter.ViewHolder>() {
        ...
        inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
            ...
            init {
                view.setOnClickListener {
                    ...
                    if (selectedCategoryIds.contains(category)) {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.tvName.text = category.replaceFirstChar { it.uppercase() }
            holder.cbCategory.isChecked = selectedCategoryIds.contains(category)
